...
from ldw_nets.lm.beam_search import BeamSearch, Hypothesis, ScorerInterface
...
class LMInterface(ScorerInterface):
    ...
    def build(cls, n_vocab: int, **kwargs):
        ...
        """
        from ldw_nets.bin.lm_train import get_parser
        def wrap(parser):
...
def torch_load_byLayer(path, model):
    """
    按层顺序加载模型权重（不依赖层名字）
    :param path: 已保存权重文件路径 (例如 'weights.pth')
    :param model: 需要加载权重的模型实例
    """
    # 加载保存的 state_dict
    state_dict = torch.load(path, map_location='cpu')

    # 提取参数张量列表
    saved_params = list(state_dict.values())
    model_params = list(model.state_dict().keys())

    # 校验参数数量是否匹配
    if len(saved_params) != len(model_params):
        print(f"[警告] 参数数量不匹配：保存的权重 {len(saved_params)} 层，模型 {len(model_params)} 层。")

    # 构造新的有序 state_dict
    new_state_dict = {}
    for i, (name) in enumerate(model_params):
        if i < len(saved_params):
            new_state_dict[name] = saved_params[i]
        else:
            print(f"[跳过] 模型参数 {name} 没有对应的权重。")
            new_state_dict[name] = model.state_dict()[name]

    # 加载新的 state_dict
    model.load_state_dict(new_state_dict, strict=False)
    print("✅ 权重已按层顺序加载完成（未使用层名匹配）")
