...
class PyTorchInference(Inference):
    def __init__(self, dims, textDecoder, initial_token_length: int):
        self.dims = dims
        ...
        key_modules = [block.attn.key for block in self.textDecoder.blocks]
        value_modules = [block.attn.value for block in self.textDecoder.blocks]
        self.kv_modules = key_modules + value_modules

    def logits(self, tokens: Tensor, audio_features: Tensor) -> Tensor:
        if not self.kv_cache:
            self.kv_cache, self.hooks = self.textDecoder.install_kv_cache_hooks(self.dims.n_text_ctx)

        if tokens.shape[-1] > self.initial_token_length:
            # only need to use the last token except in the first forward pass
            tokens = tokens[:, -1:]

        return self.textDecoder(tokens, audio_features, kv_cache=self.kv_cache)
...
class whisperDecoder():
    def __init__(self, model, dims):
        ...
        self.without_timestamps = self.options.without_timestamps
        self.prefix = self.options.prefix
        self.sample_len = dims.n_text_ctx // 2
        self.n_group = self.options.beam_size or self.options.best_of or 1
        self.n_ctx = dims.n_text_ctx

        for i in range(len(model)):
            m = model[i]
            if m.name == "TextDecoder":
                self.textDecoder = model[i]


        self.tokenizer = get_tokenizer(
            dims.is_multilingual,
            num_languages=dims.num_languages,
            language="zh",
            task="transcribe",
        )
        ...
        if not self.options.without_timestamps:
            precision = self.CHUNK_LENGTH / dims.n_audio_ctx  # usually 0.02 seconds
            ...
            self.inference = PyTorchInference(self.dims, self.textDecoder, len(self.initial_tokens))
    ...
    def _detect_language(self, audio_features: Tensor, tokens: Tensor):
        ...
        if self.options.language is None or self.options.task == "lang_id":
            lang_tokens, lang_probs = detect_language(
                self.dims, self.textDecoder, audio_features, self.tokenizer
            )
