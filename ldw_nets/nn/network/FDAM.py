...
class FlashAttn(nn.Module):
    ...
    def _flash_attn(self, x, key_padding_mask=None, need_weights=False):
        ...
        if self.ignore_cls_token > 0:
            dy_freq = torch.cat(
                [torch.zeros([B, self.ignore_cls_token, dy_freq.size(-1)], device=dy_freq.device), dy_freq], dim=1)
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., lf_dy_weight=False,
                 hf_dy_weight=True):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
...
class Layer_scale_init_Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        if window_size > 0:
            ...
            if use_flash_attn:
                ...
                self.attn = WindowedFlashAttention(
                    ...
                    use_flash_attn=use_flash_attn, causal=False, window_size=window_size)
            else:
            ...
            if use_flash_attn:
                ...
                self.attn = FlashAttn(
                    dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop,
                    use_flash_attn=use_flash_attn, causal=False)
            else:
                self.attn = Attention_block(
                    dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        ...
        self.freq_scale_1 = GroupDynamicScale(dim=dim, expansion_ratio=1, reweight_expansion_ratio=.0625, group=16,
                                              num_filters=4, size=14, act1_layer=StarReLU, act2_layer=nn.Identity,
                                              bias=False, weight_resize=True)
        self.freq_scale_2 = GroupDynamicScale(dim=dim, expansion_ratio=1, reweight_expansion_ratio=.0625, group=16,
                                              num_filters=4, size=14, act1_layer=StarReLU, act2_layer=nn.Identity,
                                              bias=False, weight_resize=True)
...
class vit_models_freq(BaseModule):
    ...
    def __init__(self, img_size=224, pretrain_img_size=224, patch_size=16, pretrain_patch_size=16, in_chans=3,
    ...
    def init_weights(self, pretrained=None):
        ...
        def resize_pos_embed(pos_embed, H, W):
            ...
            pos_embed = pos_embed.reshape(
                1, self.pretrain_img_size // self.pretrain_patch_size,
                   self.pretrain_img_size // self.pretrain_patch_size, -1).permute(0, 3, 1, 2)
