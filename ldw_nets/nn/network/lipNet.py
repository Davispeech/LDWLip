...
class LipNet(torch.nn.Module):
    def __init__(self, dropout_p=0.5):
        ...
        # 3D卷积层（保持原样）
        self.conv1 = nn.Conv3d(1, 32, (3, 5, 5), (1, 2, 2), (1, 2, 2))  # 32， 64， 96， 256， 512
        ...
        self.gru1 = nn.GRU(96*5*5, 256, 1, bidirectional=True)  # 96 * 4 * 8
    ...
    def forward(self, x):
        ...
        x = self.relu(x)
        x = self.dropout3d(x)
        x = self.pool3(x)  # torch.Size([15, 96, 77, 5, 5])

        # 调整维度（保持原样）
        x = x.permute(2, 0, 1, 3, 4).contiguous()
        x = x.view(x.size(0), x.size(1), -1)  # torch.Size([77, 15, 2400])
