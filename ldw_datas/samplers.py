...
class ByFrameCountSampler(Sampler):
    ...
    def _get_indices(self):
        if self.shuffle:  # shuffles indices corresponding to equal lengths
            ...
            return np.array(order[0]).astype(np.int64)
        # order.append(self.sizes)
        # return np.lexsort(order)[::-1]
...
class DistributedSamplerWrapper(DistributedSampler):
    ...
    sampler, and load a subset of subsampled dataset of the original dataset
...
class FixedSamplerWrapper(Sampler):
    def __init__(self, sampler):
        """
        初始化 FixedSamplerWrapper。
        参数:
            sampler: 被包装的采样器，需实现 __iter__ 方法。
        """
        super(FixedSamplerWrapper, self).__init__(None)  # 不传入数据集
        self.sampler = sampler

    def __iter__(self) -> Iterator[int]:
        """
        返回一个迭代器，按顺序返回 sampler 的索引。
        """
        # 获取原始 sampler 的索引
        original_indexes = list(self.sampler)
        return iter(original_indexes)  # 保持顺序，不随机化
