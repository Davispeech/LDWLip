...
class WarmupCosineScheduler_src(_LRScheduler):
...
class WarmupCosineScheduler(_LRScheduler):
    def __init__(self, optimizer, warmup_epochs, num_epochs, iter_per_epoch, min_lr=0.0):  # 添加 min_lr 参数，默认值为0.0
        self.base_lrs = {
            param_group["name"]: param_group["lr"]
            for param_group in optimizer.param_groups
        }
        ...
        self.iter = 0
        self.current_lr = 0
        self.min_lr = min_lr  # 保存 min_lr

        self.init_lr()  # 初始化时设置初始 lr

    def get_lr(self, base_lr):
        if self.iter < self.warmup_iter:
            lr = base_lr * self.iter / self.warmup_iter
        else:
            decay_iter = self.total_iter - self.warmup_iter
            lr = 0.5 * base_lr * (1 + np.cos(np.pi * (self.iter - self.warmup_iter) / decay_iter))
        return max(lr, self.min_lr)  # 保证 lr 不小于 min_lr
